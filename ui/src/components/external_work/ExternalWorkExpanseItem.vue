...
const hasOnSiteData = computed(() => {
  return (
    externalWorkExpanse.value.OnSiteFrom ||
    externalWorkExpanse.value.OnSiteTill
  );
});
...
<template>
  <q-card class="q-mb-lg" v-if="externalWorkExpanse">
    ...
    <q-card-sectiobeetc_settings_office_ip_addressesn horizontal class="row">
      ...
      <q-card-section class="col-3">
        <q-list>
          ...
          <ExternalWorkExpanseItemSummaryItem
            ...
            :label="`${externalworkexpanse.TotalAwayHours} ${t('LABEL_HOUR', externalworkexpanse.TotalAwayHours)}`"
          />
          <ExternalWorkExpanseItemSummaryItem
            :caption="$t('LABEL_TOTAL_WORKING_HOURS')"
            :label="`${externalworkexpanse.TotalWorkingHours} ${t('LABEL_HOUR', externalworkexpanse.TotalAwayHours)}`"
          />
          <ExternalWorkExpanseItemSummaryItem
            :caption="$t('LABEL_TOTAL_OPERATION_HOURS')"
            :label="`${externalworkexpanse.TotalOperationHours} ${t('LABEL_HOUR', externalworkexpanse.TotalAwayHours)}`"
          />
          <ExternalWorkExpanseItemSummaryItem
            :caption="$t('LABEL_TOTAL_OVERTIME_HOURS')"
            :label="`${externalworkexpanse.TotalOvertimeHours} ${t('LABEL_HOUR', externalworkexpanse.TotalAwayHours)}`"
          />
    ...
    </q-card-sectiobeetc_settings_office_ip_addressesn>
  </q-card>
</template>

...
function saveAbsenceReason() {
  ...
  if (isNewReason.value) {
    BeeTimeClock.administrationCreateAbsenceReason(selectedAbsenceReason.value).then(result => {
    ...
    BeeTimeClock.administrationUpdateAbsenceReason(selectedAbsenceReason.value.ID, selectedAbsenceReason.value).then(result => {
      if (result.status === 200) {
...
<template>
  <q-list separator>
    <q-item v-for="absenceReason in absenceReasons" :key="absenceReason.ID">
      ...
      <q-item-section side>
       <q-btn icon="edit" color="primary" @click="editAbsenceReason(absenceReason)"/>
      </q-item-section>
    </q-item>
    <q-item>
      <q-item-section>
        <q-btn class="full-width"  icon="add" :label="$t('BTN_ADD')" color="positive" @click="createAbsenceReason"/>
      </q-item-section>
    </q-item>
  </q-list>
  <q-dialog v-model="showDialog" v-if="selectedAbsenceReason">
    <q-card>
      ...
      <q-form @submit="saveAbsenceReason">
        <q-card-section>
          <q-input v-model="selectedAbsenceReason.Description" :label="$t('LABEL_DESCRIPTION')"/>
        </q-card-section>
        <q-card-section>
          <q-card-actions>
            <q-btn color="negative" :label="$t('BTN_CANCEL')" v-close-popup/>
            <q-btn color="positive" :label="isNewReason ? $t('BTN_CREATE') : $t('BTN_SAVE')" type="submit"/>
          </q-card-actions>
        </q-card-section>
      </q-form>
    </q-card>

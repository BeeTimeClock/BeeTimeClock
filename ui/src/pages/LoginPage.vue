      ...
      <q-page class="flex flex-center bg-grey-2">
        <q-card class="q-pa-md shadow-2 my_card" bordered>
          <q-card-section class="text-center">
            ...
            <div class="text-grey-9 text-h3 text-weight-bold">
              Bee Time Clock
            </div>
          ...
          <q-card-section>
            ...
            <q-btn
              v-if="authProviders?.Microsoft"
              color="primary"
              size="md"
              :label="t('LABEL_SIGN_IN_MICROSOFT')"
              no-caps
              class="full-width"
...
async function loadAuthProviders() {
  await BeeTimeClock.getAuthProviders()
    .then((result) => {
...
function loginLocal() {
  BeeTimeClock.login(email.value, password.value)
    .then((result) => {
      if (result.status === 200) {
...
function loginWithMicrosoft() {
  ...
  msalProvider.msalInstance
    .loginPopup()
    .then((result) => {
      try {
        console.log('FOund ACccount directly: ', result.account)
        console.log('Got Token: ', result.idToken);
        auth.setAccessToken(result.idToken);
        auth.setAuthProvider('microsoft');

        const accounts = msalProvider.msalInstance.getAllAccounts();
        console.log('Accounts: ', accounts)

        if (accounts != undefined && accounts.length > 0) {
          console.log('Set Account: ', accounts[0])
          msalProvider.msalInstance.setActiveAccount(accounts[0]!);
        }
      } catch (e) {

...
func (r Absence) AbsenceFindByUserIDAndBetweenDates(userID uint, start time.Time, end time.Time) ([]model.Absence, error) {
	var items []model.Absence
	db, err := r.env.DatabaseManager.GetConnection()
	if err != nil {
		return items, err
	...
	result := db.Preload(clause.Associations).Find(&items, "user_id = ? and (absence_from between ? and ? or absence_till between ? and ?)", userID, start, end, start, end)
	if result.Error != nil {
		return items, result.Error
	}
	return items, result.Error
